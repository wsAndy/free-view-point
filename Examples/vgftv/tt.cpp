...
void readCalibrationFile(char *fileName)
{
        ...
        if(pIn = fopen(fileName, "r"))
        {
            ...
            for (i=0; i<3; i++)
                        fscanf(pIn, "%lf\t%lf\t%lf",
                                &(m_CalibParams[camIdx].m_K[i][0]),
                                &(m_CalibParams[camIdx].m_K[i][1]),
                                &(m_CalibParams[camIdx].m_K[i][2]));
...
int main()
{
        char* filename = "/Users/sheng/Documents/DataSet/MSR3DVideo-Breakdancers/calibParams-breakdancers.txt";
        InitializeFromFile(filename);

  Mat dep = imread("/Users/sheng/Documents/DataSet/MSR3DVideo-Breakdancers/cam3/depth-cam3-f000.png");

  cout << dep.channels() << endl;

  uchar val = dep.at<Vec3b>(250,300)[0];
